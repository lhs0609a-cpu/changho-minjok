              ...
              {products.map((product) => (
                <div key={product.id} className="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow">
                  <div className="aspect-video bg-gradient-to-br from-hanji to-ivory flex items-center justify-center">
                    <product.icon className="w-20 h-20 text-gold" />
                  </div>
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-3">
                      <span className="px-2 py-1 bg-gold/10 text-gold text-xs font-medium rounded">{product.category}</span>
                      <span className="text-seal font-bold">{product.price}</span>
                    </div>
                    ...
                    <ul className="space-y-2 mb-6">
                      {product.features.map((feature, idx) => (
                        <li key={idx} className="flex items-center gap-2 text-sm text-ink">
                          <span className="w-1.5 h-1.5 bg-gold rounded-full" />{feature}
                        </li>
                      ))}
                    </ul>
                    <div className="flex gap-3">
                      ...
                      <Link href="/inquiry" className="flex-1 text-center py-3 bg-seal text-white rounded-lg font-medium hover:bg-seal/90 transition-colors">견적 문의</Link>
                    </div>
                  </div>
                </div>
              ))}
