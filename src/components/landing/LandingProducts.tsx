    ...
    <section className="py-20 md:py-28 bg-white">
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-6 lg:gap-8">
          {products.map((product, index) => (
            <AnimatedSection key={product.name} delay={index * 0.1}>
              <motion.div
                whileHover={{ y: -8 }}
                className="group bg-white rounded-3xl border-2 border-gray-100 hover:border-[#2AC1BC] transition-all overflow-hidden h-full"
              >
                {/* Image */}
                <div className="relative aspect-square bg-gray-50 overflow-hidden">
                  ...
                  />
                  {/* Badge */}
                  <span className="absolute top-4 right-4 px-3 py-1 bg-[#FF6F0F] text-white text-xs font-bold rounded-full">
                    {product.badge}
                  </span>
                </div>
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 rounded-xl bg-[#E8F8F7] flex items-center justify-center">
                      <product.icon className="w-5 h-5 text-[#2AC1BC]" />
                    </div>
                    <h3 className="text-xl font-bold text-[#1E1E1E]">{product.name}</h3>
                  </div>

                  <p className="text-[#767676] mb-4">{product.description}</p>
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {product.features.map((feature) => (
                      <span
                        key={feature}
                        className="px-3 py-1 bg-gray-100 text-[#767676] text-sm rounded-full"
                      >
                        {feature}
                      </span>
                    ))}
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-2 text-[#2AC1BC] font-bold group-hover:underline"
                  >
                    자세히 보기
        ...
        <AnimatedSection delay={0.4} className="text-center mt-12">
          <Link
            href="/products"
            className="inline-flex items-center gap-3 px-8 py-4 bg-[#1E1E1E] text-white rounded-xl font-bold hover:bg-[#292929] transition-colors"
          >
            전체 제품 보기
