        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16">
          ...
          <div>
            ...
            <AnimatedSection className="mb-12">
              <h3 className="text-xl font-bold text-[#1E1E1E] mb-6">공식 파트너사</h3>
              <div className="flex gap-6">
                {partners.map((partner) => (
                  <div
                    key={partner.name}
                    className="flex-1 bg-white rounded-2xl p-6 border-2 border-gray-100 flex items-center justify-center"
                  >
                    <Image
                      src={partner.logo}
                      alt={partner.name}
            ...
            <AnimatedSection delay={0.2}>
              <h3 className="text-xl font-bold text-[#1E1E1E] mb-6">인증 및 자격</h3>
                ...
                {certifications.map((cert, index) => (
                  ...
                  >
                    <div className="w-12 h-12 rounded-xl bg-[#E8F8F7] flex items-center justify-center mb-4">
                      <cert.icon className="w-6 h-6 text-[#2AC1BC]" />
                    </div>
                    <h4 className="font-bold text-[#1E1E1E] mb-1">{cert.title}</h4>
                    <p className="text-sm text-[#767676]">{cert.description}</p>
                  </motion.div>
                ))}
              </div>
            </AnimatedSection>
          ...
          <AnimatedSection direction="right" delay={0.3}>
            <div className="bg-white rounded-3xl p-8 border-2 border-gray-100 h-full">
              <h3 className="text-xl font-bold text-[#1E1E1E] mb-8">걸어온 길</h3>
                ...
                {timeline.map((item, index) => (
                  ...
                  >
                    <div className="flex flex-col items-center">
                      <div className="w-4 h-4 rounded-full bg-[#2AC1BC]" />
                      {index < timeline.length - 1 && (
                        <div className="w-0.5 h-full bg-gray-200 mt-2" />
                      )}
                    </div>
                    <div className="pb-8">
                      <span className="inline-block px-3 py-1 bg-[#E8F8F7] text-[#2AC1BC] text-sm font-bold rounded-full mb-2">
                        {item.year}
                      </span>
                      <p className="text-[#1E1E1E] font-medium">{item.event}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
